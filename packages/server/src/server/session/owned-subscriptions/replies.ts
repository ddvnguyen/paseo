...
const exceptions = {
  ...
  // Fork background-tasks request carries requestId inside payload, not top-level,
  // so isReply's `"requestId" in request` gate never treats this as a reply.
  // Listed with no originating request, like transcription_result below.
  "agent.background_tasks.list.response": [],
