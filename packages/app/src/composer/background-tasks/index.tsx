...
import type { BackgroundTaskEntry } from "@/types/stream";
...
export const BackgroundTasksTrack = memo(function BackgroundTasksTrack({
  serverId,
  agentId,
}: {
  serverId: string;
  ...
  const tasks = useSessionStore((state) =>
    state.sessions[serverId]?.agentBackgroundTasks.get(agentId),
  );
...
function BackgroundTaskRow({ task }: { task: BackgroundTaskEntry }): React.ReactElement {
  return (
...
const STATUS_DOT_STYLES: Record<BackgroundTaskEntry["status"], string> = {
  running: "#3b82f6",
  completed: "#22c55e",
  failed: "#ef4444",
  cancelled: "#6b7280",
...
function StatusDot({ status }: { status: BackgroundTaskEntry["status"] }): React.ReactElement {
  const dotStyle = useMemo(
    () => ({
      width: 8,
      height: 8,
      borderRadius: 4,
      backgroundColor: STATUS_DOT_STYLES[status],
    }),
    [status],
  );
